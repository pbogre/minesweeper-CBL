...
/**
 * The Game class extends JFrame and represents the main game window.
 */
public class Game extends JFrame {
    ...
    /**
     * The function sets the location of the JFrame 
     * to the center of the screen, sets the default close
    ...
    /**
     * The function populates the grid with bombs 
     * based on their probability of being a bomb, which is a
     * distribution where the chance of a cell being 
     * a bomb increases the further away it is from the 
     * first clicked cell. This makes it more unlikely 
     * for the first clicked cell to be surrounded by 
     * bombs and allows for a more fluent user experience 
     * since it makes it less likely to have to guess.
     * 
        ...
        for (int d = 1; d <= largestRing; d++) {
            for (int y = this.firstCell.row - d; y <= this.firstCell.row + d; y++) {
                ...
                for (int x = this.firstCell.col - d; x <= this.firstCell.col + d; x++) {
                    ...
                    double probability = currentCell.calculateProbabilityOfBomb(
                            this.firstCell.col, this.firstCell.row, 
                            this.gridSize, this.maxProbability);
    ...
    /**
     * The function populates a grid with a 
     * random number of bombs, excluding the first revealed cell.
     */
    ...
    /**
     * The function paints the probability of a cell 
     * being a bomb on the grid with the implemented distribution.
     */
    public void paintProbabilities() {
        for (int y = 0; y < this.gridSize; y++) {
            for (int x = 0; x < this.gridSize; x++) {
                ...
                double probability = currentCell.calculateProbabilityOfBomb(
                    this.firstCell.col, this.firstCell.row, this.gridSize, this.maxProbability);
     ...
     * color, foreground color, and text based on 
     * their distance from a specified cell in concetric rings
    ...
    /**
     * The function computes the number of neighboring bombs 
     * for a given cell in a Minesweeper game and
     * recursively reveals its neighbors if there are no adjacent bombs.
     * 
     * @param cell The parameter "cell" represents a 
     *      specific cell in the grid. It is an instance of the
     *      "Cell" class, which has properties such as 
     *      "row" and "col" to represent its position in
     *      the grid, as well as boolean properties 
     *      like "isRevealed", "isFlag" and "isBomb".
     */
    public void computeNeighboringBombs(Cell cell) throws GameWonException {
        ...
        if (neighboringBombs == 0) {               
            ...
            for (int i = 0; i < 8; i++) {
                ...
                // skip out of bounds
                if (currentRow < 0 || currentCol < 0 || currentRow >= this.gridSize 
    ...
    // The above code is defining a constructor 
    //for a Game class in Java. The constructor takes in two
    // parameters: gridSize and bombAmount.
    /**
     * Handles the game won event by setting the 
     * game over flag, updating the hint mode, stopping the timer, and updating the main label.
     */
    void handleGameWon() {
        this.gameOver = true;
        this.updateHintMode();
    ...
    /**
     * Constructs a new Minesweeper game with the given grid size and bomb amount.
     *
     * @param gridSize the size of the grid
     * @param bombAmount the number of bombs in the grid
     * @param maxProbability the maximum probability of a cell being a bomb
     * @param useProbability whether or not to use probability to solve the game
     * @param drawProbabilities whether or not to draw the probabilities on the cells
     * @param drawPopulationRings whether or not to draw the population rings on the cells
     */
                    ...
                    public void mouseClicked(MouseEvent me) {
                        ...
                        if (SwingUtilities.isLeftMouseButton(me)) {
                            if  (currentCell.isFlagged) {
                                ...
                                ImageIcon explosionIcon = new ImageIcon(getClass()
                                    .getResource("/res/explosion.png"));
                                currentCell.setIcon(new ImageIcon(explosionIcon.getImage()
