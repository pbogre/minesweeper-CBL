...
/**
 * The Menu class represents a menu screen for a Minesweeper game.
 */
public class Menu extends JFrame {
    ...
    public void updateCustomDifficulty() {
        ...
        this.selectedBombAmount = (int) (this.selectedGridSize * this.selectedGridSize 
            * ((double) this.bombAmountSlider.getValue() / 100.0));
        this.bombAmountLabel.setText("Bombs / Cells: " + this.bombAmountSlider.getValue() 
            + "% (" + this.selectedBombAmount + " total)");
    ...
    /**
     * The function sets the difficulty level 
     * by updating the selected grid size and bomb amount, and
     * adjusting the values of the gridSizeSlider and bombAmountSlider accordingly.
     * 
     * @param gridSize The gridSize parameter represents the size of the grid. 
     *      It determines the number of rows and columns in the grid.
     *      
     * @param bombAmount The bombAmount parameter represents 
     *      the number of bombs that will be placed on the game grid.
     */
    ...
    */
    ...
    /**
     * Constructs a new Menu object with the specified window size.
     *
     * @param windowSize The size of the window to be displayed.
     */
    public Menu(int windowSize) {
        ...
        JCheckBox useProbabilityToggle = new JCheckBox(
            "Use probability-based bomb population?", this.useProbability);
        JCheckBox drawProbabilitiesToggle = new JCheckBox(
            "Draw probability of cell being picked as bomb?", this.drawProbabilities);
        JCheckBox drawPopulationRingsToggle = new JCheckBox(
            "Draw rings used while populating bombs?", this.drawPopulationRings);

        JSlider maxProbabilitySlider = new JSlider(
            JSlider.HORIZONTAL, 0, 100, this.selectedMaxProbability);
        JLabel maxProbabilityLabel = new JLabel(
            "Max. probability: " + this.selectedMaxProbability + "%");

        JPanel difficultyPanel = new JPanel();

        this.gridSizeSlider = new JSlider(JSlider.HORIZONTAL, 5, 35, this.selectedGridSize);
        this.bombAmountSlider = new JSlider(JSlider.HORIZONTAL, 0, 99, 
        (100 * this.selectedBombAmount) / (this.selectedGridSize * this.selectedGridSize));

        this.gridSizeLabel = new JLabel("Grid size: " + gridSizeSlider.getValue());
        this.bombAmountLabel = new JLabel("Bombs / Cells: "
         + bombAmountSlider.getValue() + "% (" + this.selectedBombAmount + " total)");
        ...
        startGameButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                    ...
                    Game game = new Game(self.selectedGridSize, self.selectedBombAmount, 
                        self.selectedMaxProbability, self.useProbability, 
                        self.drawProbabilities, self.drawPopulationRings);
        ...
        optionsButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                    ...
                    boolean displayCustomMoreOptions = self.displayMoreOptions 
                        && self.useProbability;
        ...
        maxProbabilitySlider.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                ...
                maxProbabilityLabel.setText("Max. probability: " 
                    + self.selectedMaxProbability + "%");
            }
        });
    }
}
